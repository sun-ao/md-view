...
- `outline.ts` — scroll-spy outline. `findActiveIndex(tops, threshold, atBottom)` is a **pure function** (testable without jsdom layout). `waitForHeadings` polls every 100ms up to 5s (`POLL_TIMEOUT`) because Vditor renders preview HTML asynchronously. Click-to-scroll uses `animateScroll` (rAF + easeInOutCubic, 300ms) on `.vditor-preview.scrollTop` directly — native `scrollIntoView`/`scrollTo({behavior:'smooth'})` are silently no-ops in some embedded webviews/iframes, so the manual animation is required. Returns `OutlineHandle.destroy()` to detach scroll/wheel/touch listeners.
